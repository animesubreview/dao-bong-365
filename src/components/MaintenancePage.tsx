    ...
    <div className="flex items-center justify-center gap-3 mt-6">
      {[
        ...
        <div key={label} className="flex flex-col items-center">
          <div className="w-16 h-16 bg-yellow-500/20 border border-yellow-600/40 rounded-2xl flex items-center justify-center">
    ...
    <div className="fixed inset-0 z-[9999] bg-slate-950 flex flex-col items-center justify-center px-6 overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {Array.from({ length: 20 }).map((_, i) => (
          <div key={i}
            className="absolute rounded-full bg-yellow-500/5 animate-pulse"
            style={{
      ...
      <div className="relative z-10 flex flex-col items-center text-center max-w-sm">
        ...
        <div className="relative mb-5">
          <div className="w-20 h-20 bg-yellow-500/10 border border-yellow-600/30 rounded-3xl flex items-center justify-center">
            <Wrench size={36} className="text-yellow-400 animate-bounce" />
          </div>
          {/* Spinning ring */}
          <div className="absolute inset-0 rounded-3xl border-2 border-yellow-500/20 animate-spin"
        ...
        <button
          ...
          className="mt-6 flex items-center gap-2 bg-yellow-500 hover:bg-yellow-500 text-slate-950 font-black px-6 py-3 rounded-full transition-all active:scale-95 text-sm"
        >
        ...
        {config.endTime && new Date(config.endTime).getTime() > Date.now() && (
          <div className="mt-5 w-full h-1 bg-slate-800 rounded-full overflow-hidden">
            <div className="h-full bg-yellow-500 rounded-full animate-pulse"
