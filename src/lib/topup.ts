...
export const CARD_TELCOS: { value: CardTelco; label: string; color: string }[] = [
  ...
  { value: 'VIETNAMOBILE', label: 'Vietnamobile', color: 'bg-yellow-500' },
...
export function getStatusInfo(status: TopupStatus) {
  switch (status) {
    ...
    case 'pending':     return { label: 'Đang xử lý',   color: 'text-yellow-400',   bg: 'bg-yellow-950/60 border-yellow-600/30' };
