...
/** Admin tạo nhiều key cùng lúc (mã auto ngẫu nhiên, mỗi key 1 mã riêng biệt) */
export async function createVipKeysBulk(params: {
  count: number;
  tier: VipTier;
  durationMinutes: number;
  maxUses: number;
  ...
  if (params.durationMinutes <= 0) return { ok: false, codes: [], error: 'Thời hạn phải lớn hơn 0!' };
  if (params.maxUses <= 0) return { ok: false, codes: [], error: 'Số lần sử dụng phải lớn hơn 0!' };
  ...
  try {
    for (let i = 0; i < count; i++) {
      ...
      let attempts = 0;
      // Đảm bảo mã không trùng (rất hiếm khi trùng nhưng vẫn kiểm tra cho chắc)
      while (attempts < 5) {
        const existing = await getDoc(doc(db, COL, code));
      ...
      await setDoc(doc(db, COL, code), key);
      codes.push(code);
    }
    return { ok: true, codes };
  } catch (err: any) {
    return { ok: false, codes, error: 'Lỗi hệ thống: ' + (err.message || '') };
  }
}
...
export async function redeemVipKey(
  ...
  try {
    const result = await runTransaction(db, async (tx) => {
      ...
      // Mỗi tài khoản chỉ được nhập 1 key VIP duy nhất (dù admin tạo bao nhiêu key khác nhau)
      if (user.vipKeyRedeemed) {
        throw new Error('Tài khoản của bạn đã nhập key VIP rồi, mỗi tài khoản chỉ được nhập 1 lần!');
      }
      ...
      tx.update(userRef, {
        ...
        vipExpiry: newExpiry,
        vipKeyRedeemed: true,
        vipKeyRedeemedCode: key.code,
        vipKeyRedeemedAt: nowMs,
      });
