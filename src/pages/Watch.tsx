...
import PhimTuoiThoPlayer from '../components/DaoPhimPlayer';
      ...
      <AnimatePresence>
        {lastWatchRoom && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -40 }}
            className="sticky top-0 z-40 bg-indigo-600/95 backdrop-blur border-b border-indigo-500/50 px-4 py-2.5 flex items-center gap-3"
          >
      ...
      {/* ↑ đóng content-area phần 1 (để sidebar dưới đây thành ô lưới riêng, không bị lồng bên trong) */}
        ...
        {movie.content && (
          <motion.div initial={{ y: 16, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.06 }}
            ...
            <Link to={`/phim/${movie.slug}`} className="text-green-400 hover:text-green-300 text-xs font-semibold mt-2 inline-block">
              Thông tin phim ›
            </Link>
          </motion.div>
        )}
