...
function ComicCard({ comic, onClick }: { comic: Comic; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="group flex flex-col bg-slate-900 rounded-xl overflow-hidden border border-slate-800/60 hover:border-yellow-500/50 transition-all duration-200 hover:scale-[1.03] hover:shadow-xl hover:shadow-yellow-500/10 text-left"
    >
      <div className="relative w-full bg-slate-800" style={{ aspectRatio: '2/3' }}>
        ...
        <div className={cn(
          ...
          isCompleted ? 'bg-blue-600 text-white' : 'bg-yellow-500 text-white'
        )}>
          {isCompleted ? 'Full' : 'Đang ra'}
        </div>
        {latestChap && (
          <div className="absolute bottom-1.5 right-1.5">
            <span className="text-[10px] font-bold bg-black/75 text-yellow-400 px-1.5 py-0.5 rounded">
              Ch.{latestChap}
            </span>
          </div>
        )}
      ...
      <div className="px-2 py-2">
        <p className="text-white text-[11px] font-bold line-clamp-2 leading-snug group-hover:text-yellow-400 transition-colors">
          {comic.name}
        </p>
        {comic.origin_name?.[0] && (
      ...
      <div className="sticky top-0 z-30 bg-slate-950/95 backdrop-blur-xl border-b border-slate-800/60">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1 text-center min-w-0 px-2">
            ...
            <p className="text-yellow-400 text-[11px]">Chương {chap?.chapter_name}</p>
          </div>
        ...
        <div className="flex items-center gap-2 px-4 pb-3">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700/60 hover:border-yellow-500/40 rounded-xl text-xs font-bold text-slate-300 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all"
          >
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        <div className="bg-slate-900 border border-slate-800/60 rounded-2xl p-4">
          <p className="text-center text-slate-400 text-xs mb-3">
            <span className="text-white font-bold">{detail.name}</span> · Chương <span className="text-yellow-400 font-bold">{chap?.chapter_name}</span>
          </p>
          <div className="flex items-center gap-3">
            <button
              disabled={!hasPrev}
              onClick={() => goChapter(currentIndex + 1)}
              className="flex-1 flex items-center justify-center gap-2 py-3 bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-yellow-500/40 rounded-xl text-sm font-bold text-slate-300 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all"
            >
            ...
            <button
              disabled={!hasNext}
              onClick={() => goChapter(currentIndex - 1)}
              className="flex-1 flex items-center justify-center gap-2 py-3 bg-yellow-500 hover:bg-yellow-500 border border-yellow-500 rounded-xl text-sm font-bold text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-lg shadow-yellow-600/20"
            >
    ...
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      {detail.content && (
        <div className="mb-5 bg-slate-900 rounded-xl p-4 border border-slate-800/60">
          ...
          <button onClick={() => setExpanded(v => !v)} className="text-yellow-400 text-xs mt-2 hover:text-yellow-300 font-semibold">
      ...
      {allChapters.length > 0 && (
        <div className="flex gap-3 mb-5">
          <button
            ...
            className="flex-1 py-3 bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-yellow-500/40 rounded-xl text-sm font-bold text-slate-300 hover:text-white transition-all"
          >
            📖 Đọc từ đầu
          </button>
          <button
            onClick={() => handleReadChapter(allChapters.length - 1)}
            className="flex-1 py-3 bg-yellow-500 hover:bg-yellow-500 rounded-xl text-sm font-bold text-white transition-all shadow-lg shadow-yellow-600/20"
          >
            🔥 Đọc mới nhất
          </button>
        </div>
          ...
          {chaptersReversed.map((chap, i) => {
            ...
            return (
              <button key={i} onClick={() => handleReadChapter(originalIndex)}
                className="px-3 py-2.5 bg-slate-800/50 hover:bg-yellow-500/15 border border-slate-700/50 hover:border-yellow-500/40 rounded-xl text-left transition-all group">
                <p className="text-slate-200 text-xs font-semibold group-hover:text-yellow-400 transition-colors truncate">
...
export default function TruyenTranh() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 md:px-6 py-6">
        ...
        <form onSubmit={handleSearch} className="flex-1">
          <div className="relative">
            <input type="text" placeholder="Tìm tên truyện..." value={searchInput}
              ...
              className="w-full bg-slate-800/70 border border-slate-700/60 rounded-full py-2.5 pl-10 pr-9 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-yellow-500/50 transition-colors"
            />
                ...
                <div className="max-h-72 overflow-y-auto py-1.5">
                  <button onClick={() => handleGenre('')}
                    className={cn('w-full text-left px-4 py-2.5 text-sm transition-colors hover:bg-slate-800',
                      !activeGenre ? 'text-yellow-400 font-bold' : 'text-slate-300')}>
                    Tất cả thể loại
                  </button>
                  {genres.map(g => (
                    <button key={g._id} onClick={() => handleGenre(g.slug)}
                      className={cn('w-full text-left px-4 py-2.5 text-sm transition-colors hover:bg-slate-800',
                        activeGenre === g.slug ? 'text-yellow-400 font-bold bg-yellow-500/10' : 'text-slate-300')}>
                      {g.name}
                    </button>
                  ))}
                </div>
            ...
            return (
              <button key={t.key} onClick={() => handleTab(t.key)}
                className={cn(
                  ...
                  active
                    ? 'bg-yellow-500 text-white shadow-lg shadow-yellow-600/30'
      ...
      {/* Active filter */}
      {(searchQuery || activeGenre) && (
        <div className="flex items-center gap-3 mb-4 px-4 py-2.5 bg-yellow-500/10 border border-yellow-500/20 rounded-xl">
          <p className="text-yellow-300 text-sm flex-1">
            {searchQuery ? <>🔍 "<strong>{searchQuery}</strong>"</> : <>🏷 <strong>{activeGenreName}</strong></>}
          </p>
          <button onClick={resetFilters} className="flex items-center gap-1 text-xs font-bold text-yellow-400 hover:text-yellow-300 transition-colors">
            <X size={11} /> Xóa
          </button>
        </div>
      )}
            ...
            {pageNums.map(p => (
              <button key={p} onClick={() => setPage(p)}
                className={cn('w-9 h-9 rounded-lg text-sm font-bold transition-all border',
                  page === p ? 'bg-yellow-500 text-white border-transparent shadow-lg shadow-yellow-600/30' : 'bg-slate-900 text-slate-400 hover:text-white border-slate-800 hover:border-slate-700')}>
                {p}
              </button>
            ))}
