...
export default function PlayerStudio() {
  ...
  return (
    <div className="min-h-screen bg-[#060a06]" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
      ...
      {/* Header */}
      <header className="sticky top-0 z-50 border-b border-yellow-500/10 bg-[#060a06]/90 backdrop-blur-xl px-6 h-14 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-yellow-500 rounded-lg flex items-center justify-center text-base">🎬</div>
          <span className="font-bold text-white text-base">Player <span className="text-yellow-400">Studio</span></span>
          <span className="text-[10px] uppercase tracking-widest text-yellow-400/60 border border-yellow-500/20 px-2 py-0.5 rounded-full">Config</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleSave} disabled={saving}
            className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${
              saved ? 'bg-yellow-500/20 border border-yellow-500/50 text-yellow-400' :
              saving ? 'bg-yellow-500/50 text-black cursor-wait' :
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col gap-4">
          <h2 className="text-sm font-semibold text-yellow-400 uppercase tracking-widest flex items-center gap-2">
            <Eye size={14} /> Xem trước
          </h2>

          {/* Player preview */}
          <div className="relative bg-black rounded-2xl overflow-hidden border border-yellow-500/15" style={{ aspectRatio: '16/9' }}>
            {/* Fake video bg */}
          ...
          {/* Info */}
          <div className="bg-[#0d140d] border border-yellow-500/10 rounded-xl p-4 text-xs text-slate-400 space-y-1">
            ...
            <div>⌨️ Phím tắt: <strong className="text-yellow-400">Space</strong> phát/dừng, <strong className="text-yellow-400">← →</strong> tua ±10s, <strong className="text-yellow-400">F</strong> fullscreen</div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          {/* Logo Config */}
          <div className="bg-[#0d140d] border border-yellow-500/15 rounded-2xl overflow-hidden">
            <div className="px-4 py-3 border-b border-yellow-500/10 bg-[#111a11] flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-yellow-500 shadow-[0_0_6px_#22c55e]" />
              <h3 className="text-[11px] font-bold uppercase tracking-widest text-yellow-400">Logo Overlay</h3>
            </div>
            <div className="p-4 space-y-4">
              ...
              {config.logoType === 'text' && (<>
                <div>
                  ...
                  <input type="text" value={config.logoText} onChange={e => update('logoText', e.target.value)} className={inputCls} placeholder="Phim Tuổi Thơ" />
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <input type="color" value={config.logoColor1} onChange={e => update('logoColor1', e.target.value)}
                        className="w-10 h-9 rounded-lg border border-yellow-500/20 bg-[#060a06] cursor-pointer p-1" />
                    ...
                    <button key={p.val} onClick={() => update('logoPosition', p.val)}
                      className={`py-2 px-3 rounded-lg text-xs font-semibold border transition-all ${
                        config.logoPosition === p.val
                          ...
                          : 'bg-[#060a06] border-yellow-500/10 text-slate-400 hover:border-yellow-500/30'
                      }`}>
                      {p.label}
                    </button>
                  ))}
              ...
              {/* Size + Opacity */}
              <div>
                <label className={labelCls}>Cỡ chữ: <span className="text-yellow-400 font-mono">{config.logoSize}px</span></label>
              ...
              <div>
                <label className={labelCls}>Độ mờ: <span className="text-yellow-400 font-mono">{config.logoOpacity}%</span></label>
          ...
          <div className="bg-[#0d140d] border border-yellow-500/15 rounded-2xl overflow-hidden">
            <div className="px-4 py-3 border-b border-yellow-500/10 bg-[#111a11] flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-yellow-500 shadow-[0_0_6px_#22c55e]" />
              <h3 className="text-[11px] font-bold uppercase tracking-widest text-yellow-400">Cài Đặt Player</h3>
            </div>
            <div className="p-4 space-y-4">
              <div>
              ...
              <div>
                <label className={labelCls}>Âm lượng mặc định: <span className="text-yellow-400 font-mono">{config.defaultVolume}%</span></label>
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => update('autoplay', !config.autoplay)}
                  className={`w-10 h-5 rounded-full relative transition-colors ${config.autoplay ? 'bg-yellow-500' : 'bg-slate-700'}`}>
...
const inputCls = "w-full bg-[#060a06] border border-yellow-500/15 text-white font-sans text-[13px] px-3 py-2 rounded-lg outline-none focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500/20 transition-all";
const selectCls = "w-full bg-[#060a06] border border-yellow-500/15 text-white font-sans text-[13px] px-3 py-2 rounded-lg outline-none focus:border-yellow-500 transition-all";
